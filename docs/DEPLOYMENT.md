...
| Cloudflare menampilkan **502 Bad Gateway** padahal semua container `Up` | Memakai **Cloudflare Tunnel** yang diarahkan ke `https://IP:443`. Di belakang tunnel, Caddy tidak bisa mendapat sertifikat, sehingga jabat tangan TLS gagal. Isi `CADDYFILE=Caddyfile.tunnel` di `.env`, jalankan `docker compose -f docker-compose.prod.yml up -d caddy --force-recreate`, lalu ubah service tunnel menjadi **HTTP** → `localhost:80` |
| `app` atau `migrate` gagal saat `up` dengan pesan `wajib diisi di .env` untuk `ENCRYPTION_KEY` atau `SEED_ADMIN_PASSWORD` | Variabel itu memang belum ada di `.env` Anda. Compose sengaja dibuat gagal cepat di sini — lewat pola `${VAR:?...}` — daripada membiarkan container menyala lalu error saat pengguna pertama mencoba 2FA. Isi `.env` sesuai Langkah 3, lalu `up -d --build` lagi |
