...
- Node.js **20.19+** — Prisma 7.10 declares `engines.node: "^20.19 || ^22.12 || >=24.0"`. Dev host and production image both run **Node 24 LTS**. Package manager: **npm**.
- **Prisma pinned to 7.x** (`prisma@^7.10.0` + `@prisma/client@^7.10.0`), CLI and client on the same major. Never install Prisma unpinned: its npm `latest` dist-tag currently points at `8.0.0-rc.13`, a pre-release.
