  ...
  return (
    <>
      ...
      <div className={tableWrap}>
        <table data-stack className={`${table} min-w-[980px]`}>
          <thead>
            <tr>
          ...
          <tbody>
            {users.map((u) => (
              <tr key={u.id} className={u.isActive ? '' : 'opacity-60'}>
                <td data-label="Username" className={`${td} ${mono} font-semibold text-gray-900`}>{u.username}{u.id === me.id && <span className="ml-1 text-[11px] text-gray-400">(Anda)</span>}</td>
                <td data-label="Nama" className={td}>{u.name}</td>
                <td data-label="Peran" className={td}>{ROLE_LABEL[u.role]}</td>
                <td data-label="WhatsApp" className={`${td} ${mono}`}>{u.phone ? formatPhoneLocal(u.phone) : '—'}</td>
                <td data-label="2FA" className={td}><Pill on={!!u.totpEnabledAt} yes="Aktif" no="Belum" /></td>
                <td data-label="Login Terakhir" className={`${td} whitespace-nowrap`}>
                  {u.lastLoginAt ? u.lastLoginAt.toLocaleString('id-ID', { timeZone: 'Asia/Jakarta', dateStyle: 'medium', timeStyle: 'short' }) : '—'}
                </td>
                <td data-label="Status" className={td}><Pill on={u.isActive} yes="Aktif" no="Nonaktif" /></td>
