export function Kpi({ label, value, hint, tone }: { label: string; value: string; hint?: string; tone?: 'success' | 'error' | 'brand' }) {
  ...
  return (
    <div data-kpi className="min-w-0 rounded-xl border border-gray-200 bg-white p-4 max-[520px]:p-3">
      <div className="truncate text-[12px] font-semibold text-gray-500">{label}</div>
      <div data-kpi-value className={`mt-1 truncate font-mono text-[20px] font-bold max-[520px]:text-[15px] ${color}`}>{value}</div>
...
export function KpiRow({ children }: { children: React.ReactNode }) {
  return <div data-kpi-row className="mb-4 grid grid-cols-4 gap-3 max-[900px]:grid-cols-2 max-[520px]:gap-2">{children}</div>;
}
