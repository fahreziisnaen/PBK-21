export function Kpi({ label, value, hint, tone }: { label: string; value: string; hint?: string; tone?: 'success' | 'error' | 'brand' }) {
  ...
  return (
    <div className="rounded-xl border border-gray-200 bg-white p-4">
      ...
      <div className={`mt-1 whitespace-nowrap font-mono text-[20px] font-bold ${color}`}>{value}</div>
...
export function KpiRow({ children }: { children: React.ReactNode }) {
  return <div className="mb-4 grid grid-cols-4 gap-3 max-[900px]:grid-cols-2 max-[520px]:grid-cols-1">{children}</div>;
}
